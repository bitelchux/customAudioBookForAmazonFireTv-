...
let herp ="https://damp-mountain-33711.herokuapp.com/"
...
async function getMedia(book){
    ...
    let mediaEntities = []
    await fetch(herp + book.brss).then(response => response.text()).then(str => new window.DOMParser().parseFromString(str,"text/xml"))
       .then(data => {
...
{
  let mediaData = {"media":[]}
  jsonData.forEach(async (book,key, arr) =>
    {
      const mediaEntities =  await getMedia(book);
      await mediaEntities.forEach(el => mediaData.media.push(el))
      if(Object.is(arr.length -1, key))
      {
        let file = new File([JSON.stringify( mediaData)], "fileNametestfinal", {type: "application/json"});
